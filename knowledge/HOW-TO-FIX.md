...
### 12. GPU Hang: Fence Sharing Between WriteTokenKV and ComputeFlashAttentionCPU (Intel Arc A770)

**Problem**: On Intel Arc (SM6.6), layers 29-31 (CPU-weight layers in Phi-3) cause GPU hang/TDR (exit code 0xC0000005) during attention computation. The crash occurs because `ComputeFlashAttentionCPU` performs 32×2 readback operations per layer (one per head for K and V), each signaling `m_inferFence`. Meanwhile, `WriteTokenKV` also uses `m_inferFence`. This creates a fence collision where readback command lists signal later fence values than what the GPU has executed for KV writes, causing the GPU to wait indefinitely on an unsignaled fence or the driver to detect an impossible fence sequence.

**Fix**: Use a local fence inside `ComputeFlashAttentionCPU` instead of the shared `m_inferFence`:
```cpp
void ModelPipeline::ComputeFlashAttentionCPU(...) {
    ComPtr<ID3D12Fence>     attnLocalFence;
    uint64_t                attnLocalFenceValue = 0;
    HANDLE                  attnLocalEvent = nullptr;
    m_dxEngine->GetDevice()->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&attnLocalFence));
    attnLocalEvent = CreateEvent(nullptr, FALSE, FALSE, nullptr);
    // ... in readbackHead lambda:
...
This isolates each head's readback timeline from the KV write timeline, preventing fence value conflicts that trigger driver TDR on Intel GPUs.
